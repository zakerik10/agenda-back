...
@employees.route('/register/<id_business>', methods=['POST'])
@jwt_required()
def register(id_business):
    try:
        ...
        new_employee = employee_schema.load(request.get_json())
        new_employee.id_business = id_business
        
        db.session.add(new_employee)
        db.session.commit()

        return jsonify({"message": f"Empleado {new_employee.name} {new_employee.surename} registrado exitosamente."}), 201
